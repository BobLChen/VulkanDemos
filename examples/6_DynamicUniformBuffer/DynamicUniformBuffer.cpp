...
private:
    ...
    void CreateUniformBuffers()
    {
        ...
        for (int32 i = 0; i < m_MVPBuffers.size(); ++i) {
            ...
            uint32 align = Align(sizeof(m_MVPData), GetVulkanRHI()->GetDevice()->GetLimits().minUniformBufferOffsetAlignment);
            uint32 size  = align * CUBE_COUNT;
            m_MVPBuffers[i] = VulkanBuffer::CreateBuffer(VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, size);
        }
        
        m_MVPData.model.SetIdentity();
        
        m_MVPData.view.SetIdentity();
        m_MVPData.view.SetOrigin(Vector4(0, 0, -100.0f));
    ...
    void UpdateUniformBuffers()
    {
        ...
        void* dataPtr = m_MVPBuffers[m_ImageIndex]->GetMappedPtr();
        
        for (uint32 i = 0; i < CUBE_SIZE; ++i)
        {
            for (uint32 j = 0; j < CUBE_SIZE; ++j)
            {
                for (uint32 k = 0; k < CUBE_SIZE; ++k)
                {
    ...
    void LoadAssets()
    {
        m_Shader   = Shader::Create("assets/shaders/6_DynamicUniformBuffer/cube.vert.spv", "assets/shaders/6_DynamicUniformBuffer/cube.frag.spv");
        ...
        uint32 channelMask = (1 << VertexAttribute::VA_Position) | (1 << VertexAttribute::VA_Color);
